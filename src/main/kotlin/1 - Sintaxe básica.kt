class Livro(val titulo: String, val preco: Double) {
...
fun inputPreco(): Double {
    ...
    while (preco < 0) {
        print("Preço $preco inválido. Favor inserir um valor positivo. \n")
        print("Digite o preco do livro: ")
...
fun excluirLivro(repositorio: MutableList<Livro>) {
    ...
    println("Livro $livro não removido.")
}
...
fun editarLivro(repositorio: MutableList<Livro>) {
    ...
    if (livro != null) {
        ...
        println("Digite o novo título (ou pressione Enter para manter o mesmo): ")
        val novoTitulo = readLine() ?: livro.titulo
        println("Digite o novo preço (ou pressione Enter para manter o mesmo): ")
        val novoPrecoStr = readLine()
        val novoPreco = if (!novoPrecoStr.isNullOrEmpty()) novoPrecoStr.toDouble() else livro.preco
        repositorio.remove(livro)
        repositorio.add(Livro(novoTitulo, novoPreco))
